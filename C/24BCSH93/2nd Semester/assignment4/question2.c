...
Q. Write a program by using function to input the above sparse matrix you have obtained in Q1 and convert that to a general matrix form and display. Verify whether it is the same matrix you have inputted in Q1
*/
...
void create_matrix(int triplet[n][3], int matrix[n][n]) {
	...
	for (i = 0; i < triplet[0][0]; i++) {
		for (j = 0; j < triplet[0][1]; j++) {
			if (i == triplet[k][0] && j == triplet[k][1]) {
